...
namespace APPR_Astroidenator_91815
{
    public partial class Form1 : Form
    {
        ...
        private void msiQuickGuide_Click(object sender, EventArgs e)
        {
            ...
            System.Diagnostics.Process.Start(Application.StartupPath + "\\..\\..\\..\\docs\\userManual.pdf");
        ...
        private void HandleGameKeys()
        {
            ...
            if (seccondPlayerActive == false)
            {
                ...
                {
                    case "W\r": // if you want up
                        if (flakVertPosition > pnlplayingfieldSM.Top)
                        {
                            flakVertPosition = flakVertPosition - 5;
                        }
                        break;
                    case "A\r":// if you want left
                        if (pnlplayingfieldSM.Left + 6 <= flakHorPosition)
                        {
                            flakHorPosition = flakHorPosition - 7;
                        }
                        break;
                    case "S\r":// if you want down
                        if (flakVertPosition + pbxSpaceShipSM.Height < pnlplayingfieldSM.Bottom)
                        {
                            flakVertPosition = flakVertPosition + 5;
                        }
                        break;
                    case "D\r": // if you want right
                        if (pnlplayingfieldSM.Right - pbxSpaceShipSM.Width - 7 >= flakHorPosition + pbxSpaceShipSM.Width)
                        {
                            flakHorPosition = flakHorPosition + 7;
                        }
        ...
        {
            //the same as above but for the second player
            if ((PowerUp1Active == false) && (PowerUp2Active == false))
            {
        ...
        private void HitDetector()
        {
            ...
            // if the second power hits astroid
            foreach (csPhotonTorpedo m_obPhotonTorpedo2Power in obPhotonTorpedo2Power)
            {
                foreach (csAstroid m_obAstroid in obAstroid)
                {
            ...
            // if you got hit you get safe time
            else
            {
                safetime++;
                if (safetime >= 90)
            ...
            // if you touch extra hart you get one
        ...
        {
            // if they die and the score uploads
        ...
        {
            // every timer tick this all happens
            // to move player
            HandleGameKeys();
            // to check what lvl player is
            DifDetector();
            // to check if hit
            HitDetector();
            // to move thorpedo
            ControleFlakFireTorpedo();
            // to respawn astroid if they are far left
            RespawnAstroids();
            // to move the astroids
            MoveAstroid();
        }

        private void DifDetector()
        {
            // make them what they shoud be
            string m_location;
            ...
            if (tmrExplosionSM.Enabled == false)
            {
                ...
                else if (rbnSilverSM.Checked == true)
                {
                    ...
                    if (killCount == 95 && extraHeartSpawned == false)
                    {
                        SpawnHeart();
                        changeAstroids();
                    }
        ...
        // to see if they dont touch button
        ...
        // if you hit astroid they explode
